...
// -> disp
  ...
  */
  ...
  void create_screen_log_table(lv_obj_t* scr, int width, int height, int x_position, int y_position) {
    lv_obj_t* panel = lv_obj_create(scr);
    lv_obj_set_size(panel, width, height);
    lv_obj_set_pos(panel, x_position, y_position);
    lv_obj_set_style_bg_color(panel, lv_color_hex(0xFFFF00), LV_PART_MAIN);

    create_label(scr, "START SENSOR LOGGER", 10, 65);
    create_label(scr, "LOG TO FILE SYSTEM", 10, 135);
    create_label(scr, "LOG TO DATA STREAMER", 10, 205);
  }
