...
volatile bool flag_system = false; 

void IRAM_ATTR onOffSystem() {
  flag_system = true;
}
...
void setup() {
  ...
  pinMode(PIN_ON_OFF, INPUT_PULLUP); // -> нажатие - LOW, отпускание - HIGH
  esp_sleep_enable_ext0_wakeup((gpio_num_t)PIN_ON_OFF, 0); // -> пробуждение по пину GPIO 15, когда уровень будет LOW
  ...
  attachInterrupt(digitalPinToInterrupt(PIN_ON_OFF), onOffSystem, FALLING);

  /*
  1) ESP_SLEEP_WAKEUP_EXT0 -> esp32 проснулся из-за пина(GPIO 15)
  2) ESP_SLEEP_WAKEUP_EXT1 -> esp32 проснулся из-за нескольких пинов
  3) ESP_SLEEP_WAKEUP_TIMER -> esp32 проснулся по таймеру
  4) ESP_SLEEP_WAKEUP_TOUCHPAD -> esp32 проснулся из-за прикосновения touch-пина
  5) ESP_SLEEP_WAKEUP_UNDEFINED -> esp32 обычный запуск
  */

  esp_sleep_wakeup_cause_t cause = esp_sleep_get_wakeup_cause(); // -> определение причины пробуждения
  if (cause == ESP_SLEEP_WAKEUP_EXT0) {
    Serial.println("Woke up from GPIO15");
  } else {
...
void loop() {
  ...
  if (flag_system) {
    ...
    esp_deep_sleep_start(); // -> перевод в сон esp32(при использовании этой функции, все выключается)
  }
