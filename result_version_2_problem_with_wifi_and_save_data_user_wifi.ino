...
// -> disp
  ...
  // -> new block
    ...
    void load_settings() {
      if (xSemaphoreTake(settings_mutex, pdMS_TO_TICKS(100))) {
        ...
        app_settings.google_enabled = preferences.getBool("google_en", 2);
  ...
  void create_chart(lv_obj_t* scr) {
    ...
    lv_chart_set_point_count(chart, 20); // -> количество точек 
    lv_chart_set_div_line_count(chart, 5, 7); // -> деление сетки

    lv_obj_set_style_radius(chart, 0, 0); // -> без скруглений
  ...
  */
  ...
  // -> new block 2
    void screen_setting_log_mode() {
      ...
      lv_obj_align(log_menu_labels[0], LV_ALIGN_TOP_LEFT, 10, 60);
      lv_obj_align(log_menu_labels[1], LV_ALIGN_TOP_LEFT, 10, 100);
      lv_obj_align(log_menu_labels[2], LV_ALIGN_TOP_LEFT, 10, 140);
      lv_obj_align(log_menu_labels[3], LV_ALIGN_TOP_LEFT, 10, 180);
      lv_obj_align(log_menu_labels[4], LV_ALIGN_TOP_LEFT, 10, 215); // flag
