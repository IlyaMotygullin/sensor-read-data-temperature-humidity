...
volatile bool deep_sleep_flag = false;
...
// -> esp_work(блок, где данные читаются с датчиков, идет запись в google-таблицы и т.д)
  ...
  void esp_work_function(void* parameter) {
    ...
    for (;;) {
      // -> изменения
        if (deep_sleep_flag) {
          // USBSerial.println("Task is stop");
...
void turn_off_backlight() {
  for (int i = 255; i >= 0; i--) { // -> отключение подсветки
    gfx -> setBrightness(i);
    delay(3);
  }
...
void deep_sleep_function() { // -> функция для перевода esp32 в глубокий сон
  turn_off_backlight();
