...
volatile bool system_of_flag = false; // -> флаг для завершения всех задач
bool is_sleep_display = false; // -> флаг для выключения библиотеки lvgl
  ...
  void esp_work_function(void* parameter) {
    ...
    for (;;) {
      ...
      if (flag_write_to_file_system) {
        ...
        float copy_variable_tem;
        float copy_variable_hum;
...
// -> display  
  ...
  void off_esp_work(); // -> прототип ф-ии, которая отключает все таймеры и останавливает сервер
  ...
  void add_event_listner(lv_event_t* event) {
  ...
  void create_btn(lv_obj_t* scr, const char* title_btn, int width_btn, int height_btn, int position_x, int position_y) {
    lv_obj_t* btn = lv_btn_create(scr);
    lv_obj_set_size(btn, width_btn, height_btn);
    lv_obj_align(btn, LV_ALIGN_CENTER, position_x, position_y);

    lv_obj_t* label = lv_label_create(btn);
    lv_label_set_text(label, title_btn);
    lv_obj_center(label);

    lv_obj_add_event_cb(btn, add_event_listner, LV_EVENT_CLICKED, NULL);
  }
...
void off_esp_work() { // -> функция для выключения системы
  is_sleep_display = true;

  server.end(); // -> остановка работы сервера
  ...
  // -> таймемры
    if (timer_modbus) { // -> отключение timer для modbus
      timerEnd(timer_modbus);
      timer_modbus = NULL;
    }
    
    if (timer_google) { // -> отключение timer для google
      timerEnd(timer_google);
      timer_google = NULL;
    }

    if (timer_file_system) { // -> отключение timer для файловой системы
      timerEnd(timer_file_system);
      timer_file_system = NULL;
    }

    if (timer_excel) { // -> отключение timer для excel
      timerEnd(timer_excel);
      timer_excel = NULL;
    }
  // -> таймеры

  if (FT3168) { // -> Переводим тач-сенсор в режим низкого потребления
  ...
  // lv_timer_disable(lv_scr_act());
  // lv_deinit();  // -> Завершение работы LVGL

  Wire.end(); // -> завершение работы протокола I2C

  USBSerial.flush(); // -> проталкиваем данные и очищаем последовательный порт
  USBSerial.end(); // -> завершаем работу последовательного порта

  preferences.end(); // -> закрываем хранилище esp32s3
  delay(1000);

  // -> отключение экрана
  esp_deep_sleep_start(); // -> функция для перехода в глубокий сон
}
